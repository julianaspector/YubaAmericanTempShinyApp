...
yrs <-
  cdec_query("YRS", 25, "H", "2001-01-10", "2017-05-16") # data only available hourly and until 5/16/2017
lar <-
  cdec_query("AWP", 25, "E", "2001-01-10", "2017-05-16") # 15 min data is available from 2001-01-10 through the present
...
lar <-
  lar[lar$lar_parameter_value < 100 &
        lar$lar_parameter_value > 45,]
...
ui <- fluidPage(
  titlePanel(
    "Daily Averaged Water Temperatures for Lower American River (at William B Pond Park) and Lower Yuba River (at Smartsville)"
  ),
  sidebarPanel(
    fluidRow(
      column = 12,
...
server <- function(input, output, session) {
  ...
  output$linePlot <- renderPlot({
    ...
    desiredData <-
      joined_data[joined_data$month < input$dateRange[2] &
                    joined_data$month > input$dateRange[1],]
    gg <- ggplot(data = desiredData) +
      ...
      scale_color_manual(
        name = "River",
        values = c(
          "Lower American River" = "#ef8a62",
          "Lower Yuba River" = "#67a9cf"
        )
      ) +
      ylab(my.ylab) +
      xlab("Time")
  ...
  output$siteMap <- renderLeaflet({
    siteMap <-
      addMarkers(
        siteMap,
        lat = 39.2351722717285,
        lng = -121.274124145508,
        popup = "Yuba River at Smartsville"
      ) %>%
      addMarkers(siteMap,
                 lat = 38.591,
                 lng =	-121.332,
                 popup = "American River at William B Pond Park")
    siteMap
  })
}
